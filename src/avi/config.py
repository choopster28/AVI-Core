...
def load_config() -> AviConfig:
    ...
    scoring = os.getenv("AVI_SCORING", "HALF").strip().upper()
    if scoring not in {"STD", "HALF", "PPR"}:
        raise RuntimeError("AVI_SCORING must be STD, HALF, or PPR.")

    start = _int("TRANSACTION_START_WEEK", 0)
    end = _int("TRANSACTION_END_WEEK", 18)
    if start < 0 or end < start:
        raise RuntimeError("Invalid transaction week range.")

    return AviConfig(
        sleeper_league_id=league_id,
        earliest_supported_season=_int("AVI_EARLIEST_SEASON", 2024),
        transaction_start_week=start,
        transaction_end_week=end,
        expected_team_count=_int("AVI_EXPECTED_TEAM_COUNT", 16),
        minimum_completed_trades=_int("AVI_MINIMUM_COMPLETED_TRADES", 61),
        avi_season=_int("AVI_SEASON", 2026),
        avi_scoring=scoring,
        methodology_version=os.getenv("AVI_METHODOLOGY_VERSION", "2026.1").strip(),
        regular_season_start_week=_int("AVI_REGULAR_SEASON_START_WEEK", 1),
        ...
        fantasypros_max_requests_per_run=_int(
